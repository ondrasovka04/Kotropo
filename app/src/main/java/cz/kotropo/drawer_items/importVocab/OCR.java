...
public class OCR {
    ...
    private void editcv() {
        ...
        Mat horizontal_kernel = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(100, 1));
        ...
        Mat vertical_kernel = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(1, 100));
        ...
        Imgproc.drawContours(gray, contours, -1, color, 10);
        Imgproc.drawContours(gray, vertical_contours, -1, color, 10);
        Utils.matToBitmap(gray, b);
        Pix px = ReadFile.readBitmap(b);
        float a = Skew.findSkew(px);
        px = Rotate.rotate(px, a);
        Pix peng = Clip.clipRectangle(px, new Box(100, 0, px.getWidth() / 2, px.getHeight()));
        Pix pces = Clip.clipRectangle(px, new Box(px.getWidth() / 2, 0, px.getWidth() / 2, px.getHeight()));
    ...
    private void startOCR() {
        ...
        t.setPageSegMode(TessBaseAPI.PageSegMode.PSM_AUTO_OSD);
        t.setImage(b1);
